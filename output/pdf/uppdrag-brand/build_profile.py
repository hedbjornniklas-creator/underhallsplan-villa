...
(ASSETS / 'colors.json').write_text(json.dumps({'proposalVersion': '1.0', 'colors': C, 'textContrastRatios': ratios,
    'controlContrast': round(contrast('control', 'white'), 2), 'focusContrast': round(contrast('focus', 'white'), 2)}, indent=2), encoding='utf8')
(ASSETS / 'tokens.css').write_text('/* Proposed design tokens, not applied to the application. */\n:root {\n'
    + ''.join(f'  --uppdrag-{key}: {value};\n' for key, value in C.items())
    + '  --uppdrag-font: "Manrope", Arial, sans-serif;\n  --uppdrag-radius: 6px;\n'
    + '  --uppdrag-space-unit: 4px;\n  --uppdrag-control-height: 44px;\n'
    + '  --uppdrag-row-height: 56px;\n  --uppdrag-focus-width: 2px;\n'
    + '  --uppdrag-focus-offset: 2px;\n}\n', encoding='utf8')
...
def svg_logo(ink, accent, symbol_only=False, endorsed=False):
    # Open U with a detached endpoint: the work and its next handover.
    symbol = f'<path d="M12 10 V36 C12 47 19 54 30 54 H34 C45 54 52 47 52 36 V26" fill="none" stroke="{ink}" stroke-width="7" stroke-linecap="square"/><path d="M48 6 H56 V14 H48 Z" fill="{accent}"/>'
    if symbol_only:
        return f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" role="img" aria-label="Uppdrag">{symbol}</svg>'
    word, width = outlined_word('Uppdrag', 42, 78, 47, ink)
    extra = outlined_word('från HusHub', 12, 80, 70, ink, 400)[0] if endorsed else ''
    return f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {round(82 + width)} {80 if endorsed else 64}" role="img" aria-label="HusHub Uppdrag">{symbol}{word}{extra}</svg>'


for name, ink, accent, symbol, endorsed in (
    ('primary', C['ink'], C['accent'], False, False),
    ('mono', C['ink'], C['ink'], False, False),
    ('white', C['white'], C['white'], False, False),
    ('symbol', C['ink'], C['accent'], True, False),
    ('endorsed', C['ink'], C['accent'], False, True),
):
    (ASSETS / f'uppdrag-{name}.svg').write_text(svg_logo(ink, accent, symbol, endorsed), encoding='utf8')

W, H = 960, 640
PDF = OUT / 'HusHub-Uppdrag-varumarkesprofil-v1.pdf'
c = canvas.Canvas(str(PDF), pagesize=(W, H))
c.setTitle('HusHub Uppdrag | Varumärkesprofil | Förslag 1.0')
c.setAuthor('HusHub')
c.setSubject('Positionering, identitet, språk och gränssnitt. Profilförslag, inte en produktlansering.')
PAGE = 0
...
def title(kicker, headline, sub=None):
    ...
    PAGE += 1
    rect(0, 0, W, H)
    text('HusHub Uppdrag', 42, 23, 15, weight=700)
...
def end():
    line(42, 603, 918, 603)
    text('VARUMÄRKESPROFIL 1.0  /  FÖRSLAG  /  16 SEPTEMBER 2026', 42, 615, 8, 'muted')
    text(f'{PAGE:02}', 894, 613, 10, 'muted')
    c.showPage()


def symbol(x, y, scale=1, ink='ink', accent='accent'):
    c.saveState()
    c.translate(x, H - y)
    c.scale(scale, -scale)
    ...
    c.setLineCap(0)
    path = c.beginPath()
    path.moveTo(12, 10); path.lineTo(12, 36)
    path.curveTo(12, 47, 19, 54, 30, 54); path.lineTo(34, 54)
    path.curveTo(45, 54, 52, 47, 52, 36); path.lineTo(52, 26)
    c.drawPath(path)
    c.setFillColor(color(accent))
    c.rect(48, 6, 8, 8, fill=1, stroke=0)
    c.restoreState()


def logo(x, y, scale=1, ink='ink', accent='accent', endorsed=False):
    ...
    c.drawString(x + 78 * scale, H - y - 47 * scale, 'Uppdrag')
    if endorsed:
...
rect(0, 178, 960, 270, 'paper')
rect(42, 203, 5, 213, 'accent')
logo(65, 238, 1.35, endorsed=True)
p('Samla arbetet.<br/>Få det gjort.', 530, 212, 380, 34, weight=700)
p('Från första notering till avslutat uppdrag.', 532, 330, 365, 16, 'muted')
button('Öppna uppdrag', 533, 386, 207)
for i, (key, label) in enumerate((('ink', 'Grafit'), ('accent', 'Uppdragsgul'), ('paper', 'Ljus arbetsyta'), ('link', 'Petrol'))):
    x = 42 + i * 224
...
for y, head, body in (
    ...
    (306, 'Uppdrag', 'Modulen och arbetsytan. I extern text: HusHub Uppdrag.'),
    (387, 'Gizmo', 'Assistenten inne i arbetsytan. Föreslår och följer upp; användaren beslutar.'),
):
...
title('Logotyp', 'Uppdrag först. HusHub som avsändare.',
      'Nytt logotypförslag: ett öppet U med en separat slutpunkt som antyder nästa överlämning.')
...
symbol(506, 347, .85)
text('24 px', 590, 363, 12, 'muted')
symbol(662, 348, .5)
text('Små format provas separat', 711, 360, 11, 'muted')
p('<b>Frizon:</b> minst en halv symbolbredd.<br/><b>Minsta bredd:</b> ordmärke 120 px / 30 mm i tryck. Med avsändarrad minst 180 px. Symbol minst 24 px.', 42, 448, 416, 13)
p('<b>Använd inte:</b> gradienter, skuggor, utsträckning eller en ny färg per vy. Den gula punkten är en detalj, aldrig en statusindikator.', 500, 448, 412, 13)
p('SVG-filerna har ordmärken som banor. Förslaget är inte varumärkesrättsligt granskat och ersätter inte befintlig logotyp utan ett införandebeslut.', 42, 557, 874, 11, 'muted')
end()

# 04: Color and semantic status.
title('Färger', 'Neutral arbetsyta. Gul igenkänning.',
      'Grafit är huvudhandling. Gult ger karaktär. Petrol används sparsamt till länkar och fokus.')
for i, (key, label) in enumerate((('ink', 'Grafit'), ('accent', 'Uppdragsgul'), ('white', 'Vitt'), ('paper', 'Arbetsyta'), ('muted', 'Sekundär text'), ('link', 'Petrol'))):
    x = 42 + i * 147
    rect(x, 183, 135, 68, key, stroke='line' if key in ('white', 'paper') else None)
    text(label, x, 264, 11, weight=600)
    text(C[key], x, 286, 11, 'muted')
text('Grundregel', 42, 334, 20, weight=700)
p('Låt cirka 85 % vara vitt och neutralt. Använd grafit för struktur och viktiga handlingar. Gult är en liten accent, inte en bakgrund för hela modulen.', 42, 372, 410, 13)
...
# 05: Typography and control geometry.
...
examples = [
    ...
    ('Gizmo föreslår 12 timmar', 'AI:n har räknat ut rätt pris', 'Förslag ska inte se ut som kontrollerade fakta.'),
]
...
# 07: Assistant, authority and system status.
title('Gizmo & förtroende', 'AI hjälper till. Människan ger klartecken.',
      'Gizmo är en diskret assistent inom Uppdrag. Ingen egen maskot, parallell färgvärld eller konkurrerande logotyp.')
text('Ett användbart AI-förslag', 42, 185, 20, weight=700)
rect(42, 226, 416, 211, 'paper', 6, 'line')
text('Gizmo föreslår', 59, 241, 12, 'muted', 600)
...
p('Fiktivt exempel. Förslag visar antaganden och källa där det är relevant. Ett accepterat AI-förslag är inte automatiskt en skickad beställning.', 42, 458, 416, 12)
text('Systemets språk måste vara sant', 505, 185, 20, weight=700)
states = [
    ('Arbetar', 'Gizmo förbereder ett förslag. Ingen procentsats utan verkligt mätbara framsteg.'),
...
# 08: Desktop visual application, deliberately not a full app implementation.
...
# 09: Mobile visual application and accessibility intent.
...
# 10: External recipient and brand boundaries.
title('Mejl & dokument', 'Mottagaren ska förstå ärendet, inte vårt system.',
      'Den som begär pris är avsändare. HusHub Uppdrag är verktyget. Gizmo är inte avtalspart.')
...
text('Skickat via HusHub Uppdrag', 64, 510, 10, 'muted')
...
p('Vit bakgrund, grafitrubriker och en liten gul accent. PDF:er ska fungera även i svartvitt. Låt kundens eller avsändarens identitet få utrymme.', 532, 368, 377, 13)
...
rect(42, 184, 423, 360, 'paper', 6)
logo(61, 201, .55, endorsed=True)
p('Samla arbetet.<br/>Få det gjort.', 67, 288, 370, 32, weight=700)
p('Underlag, ansvar och uppföljning.<br/>På samma plats.', 67, 391, 364, 16)
rect(67, 475, 34, 5, 'accent')
text('HusHub Uppdrag', 67, 502, 12, weight=600)
...
p('<b>Undvik:</b> handslag, robotbilder, glödande AI-effekter, påhittade kundomdömen, täta collage och färger som byter betydelse mellan vyer.', 507, 496, 403, 12)
text('Exempel på socialt inlägg. Inga pris- eller funktionslöften.', 42, 562, 10, 'muted')
end()

# 12: Deliverables and implementation gates.
title('Leverans & införande', 'En sammanhållen profil. Ett avgränsat införande.',
      'Samma struktur som RenoApp-profilen, men en egen identitet för uppdragsarbetet.')
text('Profilpaketet innehåller', 42, 184, 20, weight=700)
p('• Denna varumärkesprofil i 12 sidor.<br/>• Primär, enfärgad, vit och avsändarmärkt logotyp.<br/>• Fristående symbol i SVG.<br/>• Manrope-font med SIL OFL-licens.<br/>• CSS-variabler och kontrollerade färgpar.<br/>• Koncept för dator, mobil, mejl och marknadsföring.', 42, 225, 425, 13)
text('Godkänn och inför i denna ordning', 505, 184, 19, weight=700)
p('1. Namnhierarki, logotyp och färgriktning.<br/>2. Uppdragsmodulens listor och sidopaneler.<br/>3. Förfrågningar och mottagarvyer.<br/>4. Mejl, PDF:er och marknadsföring.<br/>5. Testa skärmbredder, kontrast, fokus och status.', 505, 225, 410, 13)
p('Behåll HusHubs gemensamma komponenter och meddelandesystem. Avgränsa temaändringar till Uppdrag så att RenoApp, ÖB, EB och TU inte påverkas.', 505, 365, 410, 12)
text('Underlag och källor', 42, 414, 18, weight=700)
p('RenoApp-varumärkesprofil 1.0 (14 september 2026). Befintlig Uppdrag-kod, Gizmo-namnet och ert offertarbete. Konceptvyerna är illustrerade, inte appskärmbilder.', 42, 448, 425, 11, 'muted')
links = [
...
p('Ingen produktkod, publicerad prototyp eller befintlig logotyp har ändrats genom denna leverans.', 54, 554, 399, 10)
end()
...
with zipfile.ZipFile(OUT / 'HusHub-Uppdrag-profilpaket-v1.zip', 'w', zipfile.ZIP_DEFLATED) as archive:
