...
import Landing from '../../src/app/renoapp/page'
import RequestAccess from '../../src/app/renoapp/request-access/page'
import Terms from '../../src/app/renoapp/villkor/brf/page'

const path = location.pathname
const login = path.endsWith('/login')
const legacy = path === '/legacy'
const publicPage = path === '/renoapp' ? <Landing /> : path === '/renoapp/request-access' ? <RequestAccess /> : path === '/renoapp/villkor/brf' ? <Terms /> : null
const page = path.includes('/cases/') ? <CaseDetail /> : path.endsWith('/brf') ? <Brf /> : path.endsWith('/users') ? <Users /> : <Overview />
createRoot(document.getElementById('root')!).render(publicPage ?? (legacy ? <PasswordAuthPanel /> : (
