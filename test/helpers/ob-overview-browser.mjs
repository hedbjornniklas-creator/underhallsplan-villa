...
export async function testOverview(base, output) {
  ...
  async function loadedQuery(expected) {
    await page.waitForFunction(values => {
      const query = new URLSearchParams(window.__obOverviewTest.urls.at(-1))
      return Object.entries(values).every(([key, value]) => query.get(key) === value)
    }, {}, expected)
    // The effect starts fetch before React commits its loading state; let that commit happen first.
  ...
  try {
    ...
    await loadedQuery({ search: 'lindvägen', page: '1' })
    ...
    await loadedQuery({ search: '' })
    await page.type('[aria-label="Sök uppdrag"]', '2026-0925-01')
    await loadedQuery({ search: '2026-0925-01' })
    ...
    await loadedQuery({ search: '', filter: 'all', page: '1' })
    ...
    assert.equal(await page.evaluate(() => window.__obOverviewTest.reads), initialReads, 'Fresh data suppresses automatic refresh')
    await page.evaluate(() => {
      window.__obOverviewTest.clockOffset += 5_001
      window.dispatchEvent(new Event('focus'))
      document.dispatchEvent(new Event('visibilitychange'))
    })
    await page.waitForFunction(count => window.__obOverviewTest.reads > count, {}, initialReads)
    assert.equal(await page.evaluate(() => window.__obOverviewTest.reads), initialReads + 1, 'Focus and visibility share one in-flight request')
    await page.waitForFunction(() => document.querySelector('.obo').getAttribute('aria-busy') === 'false')
    ...
    assert.equal(await page.evaluate(() => window.__obOverviewTest.pageLengths[0]), 10, 'Only the first server page is returned initially')
    assert.equal(await page.evaluate(() => window.__obOverviewTest.pageLengths.every(length => length <= 25)), true)
    ...
    await ready()
    await page.evaluate(() => {
      window.__obOverviewTest.fail = true
      window.__obOverviewTest.delay = 500
      window.__obOverviewTest.clockOffset += 5_001
      window.dispatchEvent(new Event('focus'))
    })
    await page.evaluate(() => { window.__obOverviewTest.fail = false; window.__obOverviewTest.delay = 0 })
    await page.type('[aria-label="Sök uppdrag"]', 'lindvägen')
    await loadedQuery({ search: 'lindvägen' })
