...
export async function testRoundParity({ page, click, fresh, fill, noOverflow, output }) {
  ...
  async function imageSheet() {
    ...
    assert.equal(await page.$('dialog .obm-delete-icon'), null, 'linking has no destructive action')
  }
  async function deleteImage() {
    await page.click('.obm-image-thumb')
    await waitSheet('Bild')
    await page.click('dialog .obm-image-actions summary')
    await noOverflow()
    await page.screenshot({ path: resolve(output, `image-actions-${page.viewport().width}.png`) })
    await click('Radera bild från besiktningen', 'dialog')
  ...
  const beforeCancel = await page.evaluate(() => JSON.stringify({ images: window.__obMobileTest.images, notes: window.__obMobileTest.notes, calls: window.__obMobileTest.calls }))
  await click('Avbryt', 'dialog footer')
  await closed()
  assert.equal(await page.evaluate(() => JSON.stringify({ images: window.__obMobileTest.images, notes: window.__obMobileTest.notes, calls: window.__obMobileTest.calls })), beforeCancel)
  await deleteImage()
  await page.waitForFunction(() => !document.querySelector('dialog .obm-danger').disabled)
  await click('Avbryt', 'dialog')
  await waitSheet('Bild')
  ...
  for (const width of [320, 360, 390, 430, 1280]) {
    ...
    await fresh()
    await imageSheet()
    await page.screenshot({ path: resolve(output, `link-image-cancel-${width}.png`) })
    await click('Avbryt', 'dialog footer')
    await closed()
    await deleteImage()
    await noOverflow()
