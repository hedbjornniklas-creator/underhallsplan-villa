...
export async function testImagePreview(page, base, output) {
  ...
  for (const query of ['?locked', '?queued']) {
    ...
    await page.click(`${viewer} .obm-image-actions summary`)
    assert.equal(await page.$eval(`${viewer} .obm-image-actions button`, node => node.disabled), true, 'image removal is blocked for locked inspections and queued uploads')
    assert.equal(await page.evaluate(() => window.__obMobileTest.calls.length), 0)
    await page.click(`${viewer} [aria-label="Tillbaka"]`)
    await page.waitForFunction(() => !document.querySelector('dialog[open]'))
  }
  ...
  await page.click(`${linker} footer .obm-primary`)
  await closed()
