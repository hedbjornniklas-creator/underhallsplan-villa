...
function harness({ quote = {}, quoteDocument = false, failure = false, missingFile = false, tooLarge = false } = {}) {
  const calls = [], reads = [], downloads = [], sent = []
  const q = { id: id(5), supplier_email: 'ue@example.test', updated_at: 'v1', request_subject: 'Subject', request_body: 'Repair <here>', request_attachment_ids: [id(6)], ...quote }
  const admin = {
    from(table) {
      ...
      const result = () => table === 'profiles' ? { data: { email: 'inspector@example.test' } }
        : table === 'action_case_attachments' ? { data: missingFile ? [] : [{ id: id(6), file_name: 'photo.jpg', file_size_bytes: tooLarge ? 6000000 : 5, storage_bucket: 'private', file_path: 'allowed-photo', content_type: 'image/jpeg' }] }
          : { data: read.columns === 'document_id' ? quoteDocument ? [{ document_id: id(6) }] : [] : q }
  ...
  const mod = { exports: {} }
  new Function('module', 'exports', 'require', 'process', code)(mod, mod.exports, (name) => {
  ...
  }, { env: { ASSIGNMENTS_MAIL_FROM: 'sender@example.test', RESEND_API_KEY: 'test-only' } })
  const run = (overrides = {}) => mod.exports.sendQuoteRequest({ orgId: id(1), userId: id(2) }, { caseId: id(3), itemId: id(4), costLineId: id(8), quoteId: id(5), expectedQuoteUpdatedAt: 'v1', confirmSend: true, ...overrides })
