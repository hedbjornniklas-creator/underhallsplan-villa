...
test('round section aliases remain recognized; ordinary links keep the original start page', () => {
  ...
  for (const section of ['insida', 'utsida', 'runda', 'runda-ny']) {
    assert.equal(getInitialObSection(`?section=${section}`), 'runda-ny')
  }
  assert.equal(getInitialObSection('?section=unknown'), 'grunddata')
...
test('production integration uses existing writes and the draft guard, not the test-copy adapter', () => {
  ...
  assert.match(page, /\['runda', 'insida', 'utsida'\]\.includes\(activeSection\)/)
  assert.match(wizard, /case 'runda':\s+case 'runda-ny':\s+case 'utsida':\s+case 'insida':\s+return <ObStepRunda/)
  assert.match(wizard, /mobileLayout\s+address=/)
  assert.doesNotMatch(wizard, /import ObStep(?:Insida|Utsida)/)
