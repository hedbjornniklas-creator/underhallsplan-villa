...
import { isFullRequest, normalizeQuoteRequest, REQUEST_REQUIREMENTS, requestCategoryLabel, requestLineText, requestSources } from '@/lib/action-cases/quoteRequests'
...
export function ActionCaseRequestsPanel({ actionCase, busy, onOpen }: { actionCase: ActionCaseView; busy: boolean; onOpen: (id: string | null) => void }) {
  return <section className="border-b border-slate-200 bg-white p-5" aria-label="Offertförfrågningar">
    ...
    {!actionCase.quoteRequests?.length ? <p className="mt-3 text-sm text-slate-500">Inga samlade förfrågningar ännu.</p> : <ul className="mt-3 divide-y divide-slate-200">{actionCase.quoteRequests.map((r) => <li key={r.id}><button disabled={busy} className="grid w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-3 py-3 text-left hover:bg-slate-50" onClick={() => onOpen(r.id)}><span className="min-w-0"><strong className="block break-words text-sm">{r.supplierName}</strong><span className="mt-1 block text-xs text-slate-500">{r.lines.length} delar · {statuses[r.deliveryStatus]}{r.supplementsId ? ' · Komplettering' : ''}</span></span><ArrowRight size={17} /></button></li>)}</ul>}
  </section>
}
...
export default function ActionCaseRequestSheet({ actionCase, requestId, preselectedLineId, preselectedLineIds, supplementId, busy, people = [], onClose, onAction, onSupplement, onOpenWork }: Props) {
  ...
  const initialLineIds = preselectedLineIds ?? (preselectedLineId ? [preselectedLineId] : [])
  ...
  const [form, setForm] = useState(() => ({
    ...
    selectedIds: request?.lines.map((l) => l.costLineId) ?? initialLineIds,
    pricePresentation: request?.pricePresentation ?? (request ? 'itemized' : 'action_total'),
    requirementKeys: base?.requirements.map((r) => r.key) ?? [], otherRequirements: base?.otherRequirements ?? '',
    attachmentIds: request?.attachmentIds ?? defaultsFor(initialLineIds), supplementsId: request?.supplementsId ?? supplementId ?? null,
  }))
  ...
  const selectWork = (ids: string[], checked: boolean) => {
    setForm((current) => {
      const next = checked ? [...new Set([...current.selectedIds, ...ids])] : current.selectedIds.filter((id) => !ids.includes(id))
      return { ...current, selectedIds: next, attachmentIds: reconcileRequestAttachments(current.attachmentIds, defaultsFor(current.selectedIds), defaultsFor(next), attachmentOverrides) }
    })
    setDirty(true)
  }
  let valid = selectedLines.length === form.selectedIds.length
  if (!isFullRequest(form.pricePresentation) && selectedLines.some((line) => !['own_labor', 'subcontractor'].includes(line.category ?? ''))) valid = false
  try { normalizeQuoteRequest({ ...form, lines: selectedLines }); } catch { valid = false }
  const current = request && request.lines.every((line) => sources.some((s) => s.costLineId === line.costLineId && s.itemId === line.itemId && s.scope === line.scope && s.itemTitle === line.itemTitle && s.description === line.description && (s.workPartId ?? null) === (line.workPartId ?? null) && (s.workPartTitle ?? '') === (line.workPartTitle ?? '') && (s.workPartScope ?? '') === (line.workPartScope ?? '') && (locked || !isFullRequest(request.pricePresentation) || (s.category === line.category && s.quantity === line.quantity && s.unit === line.unit && s.quantityBasis === line.quantityBasis))))
  ...
  return createPortal(<div className="uppdrag-scope fixed inset-0 z-[70] flex justify-end bg-black/40"><div ref={dialog} role="dialog" aria-busy={working} aria-modal="true" aria-labelledby="group-request-title" tabIndex={-1} className="flex h-dvh w-full max-w-3xl flex-col bg-white text-slate-950 shadow-2xl outline-none" onKeyDown={(e) => {
    e.stopPropagation()
    ...
    <div data-request-content className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6">
      {editing ? <form id="group-request-form" className="space-y-6" onSubmit={(e) => { e.preventDefault(); if (!valid || working) return; void run('quote_request', { ...form, operation: 'save', lines: selectedLines, expectedUpdatedAt: expected }).then((ok) => { if (ok) { setDirty(false); setEditing(false) } }) }}>
        ...
        <fieldset disabled={working} className="border-t border-slate-200 pt-4"><legend className="font-semibold">Delar som ska ingå *</legend>
          {sources.length ? <label className="mt-2 flex min-h-11 items-center gap-3 text-sm font-semibold"><input type="checkbox" className="h-4 w-4 shrink-0 accent-violet-600" checked={sources.every((line) => form.selectedIds.includes(line.costLineId))} onChange={(e) => selectWork(sources.map((line) => line.costLineId), e.target.checked)} />Välj alla delar</label> : null}
          {actionCase.items.map((item) => {
            const rows = sources.filter((s) => s.itemId === item.id)
            const parts = [null, ...(item.workParts ?? [])]
            return rows.length ? <section key={item.id} className="mt-3 border-b border-slate-200 pb-3">
              <label className="flex min-h-11 items-center gap-3 text-sm font-semibold"><input type="checkbox" className="h-4 w-4 shrink-0 accent-violet-600" checked={rows.every((line) => form.selectedIds.includes(line.costLineId))} onChange={(e) => selectWork(rows.map((line) => line.costLineId), e.target.checked)} /><span className="min-w-0 break-words">{item.title}</span></label>
              {parts.map((part) => {
                const partRows = rows.filter((line) => (line.workPartId ?? null) === (part?.id ?? null))
                return partRows.length ? <div key={part?.id ?? 'unassigned'} className="ml-7">
                  {part ? <label className="flex min-h-11 items-center gap-3 text-sm font-semibold"><input type="checkbox" className="h-4 w-4 shrink-0 accent-violet-600" checked={partRows.every((line) => form.selectedIds.includes(line.costLineId))} onChange={(e) => selectWork(partRows.map((line) => line.costLineId), e.target.checked)} /><span className="min-w-0 break-words">{part.title}</span></label> : null}
                  {['Arbete', 'Material', 'Övrigt'].map((category) => {
                    const categoryRows = partRows.filter((line) => requestCategoryLabel(line.category) === category)
                    return categoryRows.length ? <fieldset key={category} className="mt-2"><legend className="text-xs font-semibold text-slate-500">{category}</legend>{categoryRows.map((line) => <label className="flex min-h-11 items-center gap-3 py-1 text-sm text-slate-600" key={line.costLineId}><input className="h-4 w-4 shrink-0 accent-violet-600" type="checkbox" name="workLine" value={line.costLineId} checked={form.selectedIds.includes(line.costLineId)} onChange={(e) => toggle('selectedIds', line.costLineId, e.target.checked)} /><span className="min-w-0 break-words">{requestLineText(line)}</span></label>)}</fieldset> : null
                  })}
                </div> : null
              })}
            </section> : null
          })}
          {!sources.length ? <p className="mt-3 text-sm text-amber-800">Det saknas kalkylrader i åtgärderna.</p> : null}
          {form.selectedIds.length > 30 ? <p role="status" className="mt-3 text-sm text-amber-800">Välj högst 30 delar per förfrågan.</p> : null}
        ...
        <fieldset disabled={working} className="border-t border-slate-200 pt-4"><legend className="font-semibold">Begär pris</legend>
          <label className="mt-2 flex min-h-11 items-center gap-3 text-sm"><input type="radio" name="pricePresentation" className="h-4 w-4 shrink-0 accent-violet-600" value="action_total" checked={form.pricePresentation === 'action_total'} onChange={() => set('pricePresentation', 'action_total')} />Totalpris per åtgärd</label>
          <label className="flex min-h-11 items-center gap-3 text-sm"><input type="radio" name="pricePresentation" className="h-4 w-4 shrink-0 accent-violet-600" value="line_items" checked={form.pricePresentation === 'line_items'} onChange={() => set('pricePresentation', 'line_items')} />Pris per kalkylrad</label>
          {request && !isFullRequest(request.pricePresentation) ? <label className="flex min-h-11 items-center gap-3 text-sm"><input type="radio" name="pricePresentation" className="h-4 w-4 shrink-0 accent-violet-600" value={request.pricePresentation ?? 'itemized'} checked={!isFullRequest(form.pricePresentation)} onChange={() => set('pricePresentation', request.pricePresentation ?? 'itemized')} />Tidigare prisindelning (endast arbete)</label> : null}
        </fieldset>
        {(['included', 'separate'] as const).map((kind) => <fieldset disabled={working} className="border-t border-slate-200 pt-4" key={kind}><legend className="font-semibold">{kind === 'included' ? 'Ytterligare krav på priset' : 'Separata prisuppgifter'}</legend><div className="mt-2 grid gap-x-4 sm:grid-cols-2">{REQUEST_REQUIREMENTS.filter((r) => r.kind === kind).map((r) => <label className="flex min-h-11 items-center gap-3 py-2 text-sm" key={r.key}><input className="h-4 w-4 shrink-0 accent-violet-600" type="checkbox" name="requirement" value={r.key} checked={form.requirementKeys.includes(r.key)} onChange={(e) => toggle('requirementKeys', r.key, e.target.checked)} /><span>{r.label}</span></label>)}</div></fieldset>)}
        ...
        {request.sentAt ? <section className="border-t border-slate-200 pt-4" aria-label="Offertsvar"><header className="flex flex-wrap items-center justify-between gap-2"><h3 className="font-semibold">Offertsvar</h3><button className={secondary} disabled={working || responseEditing || priceEditing || actionCase.quotePackages?.some((price) => price.requestId === request.id && price.state === 'active')} onClick={() => setResponseEditing(true)}><Pencil size={16} />{request.pricePresentation !== 'itemized' ? 'Offertuppgifter' : 'Registrera prisvillkor'}</button></header>
          {responseEditing ? <ResponseForm request={request} actionCase={actionCase} busy={working} onSave={(data) => run('quote_request', data)} onCancel={() => setResponseEditing(false)} /> : <>
            {request.pricePresentation === 'itemized' || request.responseMode !== 'pending' ? <p className={`mt-3 text-sm ${request.responseMode === 'itemized' ? 'text-slate-700' : 'text-amber-800'}`}>{request.responseMode === 'itemized' ? 'Delpriser gäller vid separat beställning.' : request.responseMode === 'package' ? `Gemensamt paketpris: ${request.packageAmount === null ? 'belopp saknas' : money(request.packageAmount)}. Inte ett fristående delpris.` : 'Bekräfta om delpriserna gäller vid separat beställning innan de används i kalkylen.'}</p> : null}
            ...
            <ActionCaseGroupPrices actionCase={actionCase} request={request} busy={working} onEditing={setPriceEditing} onAction={run} />
            {!isFullRequest(request.pricePresentation) ? <details className="mt-3 border-t border-slate-200 pt-3" open={request.pricePresentation !== 'grouped' || undefined}><summary className="min-h-11 cursor-pointer text-sm font-semibold">Enskilda radpriser</summary><ul className="divide-y divide-slate-200">{request.lines.map((s) => { const item = actionCase.items.find((i) => i.id === s.itemId), line = item?.costLines.find((l) => l.id === s.costLineId), quote = line?.quotes?.find((q) => q.requestId === request.id); return <li className="flex flex-wrap items-center justify-between gap-2 py-3" key={s.costLineId}><div className="min-w-0"><strong className="block break-words text-sm">{s.itemTitle}: {s.description}</strong><p className="mt-1 text-xs text-slate-500">{line?.coveredByQuoteId || quote?.packageGroupKey ? 'Ingår i grupppris' : quote?.amount == null ? 'Pris saknas' : `${money(quote.amount)} · ${quote.checked ? 'Kontrollerat' : 'Ej kontrollerat'}`}</p></div><button className={secondary} disabled={working || priceEditing || !line} onClick={() => onOpenWork(s.itemId, s.costLineId)}>Öppna kalkyl<ArrowRight size={16} /></button></li> })}</ul></details> : null}
          </>}
        </section> : null}
