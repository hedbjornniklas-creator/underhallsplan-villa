...
export function ActionCaseAttachmentImage({ caseId, file, eager = false }: { caseId: string; file: ActionCaseAttachmentView; eager?: boolean }) {
  const [status, setStatus] = useState<'loading' | 'ready' | 'error'>('loading')
  return <>
    {status !== 'error' ? <Image src={actionCaseImageUrl(caseId, file.id)} alt={file.title || file.fileName} fill unoptimized loading={eager ? 'eager' : 'lazy'} className="object-contain" onLoad={() => setStatus('ready')} onError={() => setStatus('error')} /> : null}
