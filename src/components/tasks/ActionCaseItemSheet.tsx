...
import { ArrowRight, Check, Hammer, Loader2, Package, Pencil, Plus, Save, Sparkles, Trash2, Truck, UsersRound, X } from 'lucide-react'
import type { ActionCaseCostLineView, ActionCaseCostSuggestion, ActionCaseItemView } from '@/lib/action-cases/contracts'
...
const groups = [
  { key: 'own_labor', title: 'Arbete', icon: Hammer, categories: ['own_labor'], ready: 'ownLaborReady' },
  { key: 'material', title: 'Material', icon: Package, categories: ['material'], ready: 'materialPriceReady' },
  { key: 'subcontractor', title: 'Underentreprenör', icon: UsersRound, categories: ['subcontractor'], ready: 'subcontractorPriceReady' },
...
function CostLineForm({ initial, category, busy, onSave, onCancel }: {
  ...
  return <form className="space-y-4 border-y border-violet-200 bg-violet-50/30 py-4" onSubmit={(e) => {
    ...
    void onSave({ ...cost, costLineId: initial?.id }).then((saved) => { if (saved) onCancel() })
  }}>
    ...
    <fieldset disabled={busy} className="grid min-w-0 gap-3 sm:grid-cols-2">
      ...
      <label className="text-xs font-semibold text-slate-600">Kategori<select className={inputClass} value={cost.category} onChange={(e) => set('category', e.target.value)}>{Object.entries(categoryLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
...
export default function ActionCaseItemSheet({ item, busy, onClose, onSave, onCostAction }: Props) {
  ...
  const close = () => {
    if (working) return
    if ((dirty || editing) && !window.confirm('Stäng utan att spara ändringarna?')) return
    onClose()
  }
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6">
        {tab === 'scope' ? <fieldset disabled={working} className="space-y-5">
          ...
          {item.costSuggestion && !generating ? <Proposal key={item.costSuggestion.id} proposal={item.costSuggestion} stale={stale || dirty} busy={working || Boolean(editing)} onApply={(lineIds) => run(() => onCostAction('apply_cost_suggestions', { suggestionId: item.costSuggestion!.id, lineIds }))} /> : null}
          <div className="mt-5 space-y-6">{groups.map((group) => {
            ...
            const noNeed = group.key === 'subcontractor' ? !item.requiresSubcontractor : item[group.ready]
            return <section key={group.key} className="border-t border-slate-200 pt-4">
              <header className="flex flex-wrap items-center gap-2"><Icon size={18} className="text-slate-500" /><h3 className="mr-auto font-semibold">{group.title} <span className="font-normal text-slate-400">{lines.length}</span></h3>
                {lines.length > 0 ? <span className="text-xs text-slate-600">Kostnad {amount(groupTotals.internalCost)}</span> : <label className="flex items-center gap-2 text-xs text-slate-600"><input type="checkbox" disabled={working || dirty} className="h-4 w-4 accent-violet-600" checked={noNeed} onChange={(e) => void run(() => onSave(group.key === 'subcontractor' ? { requiresSubcontractor: !e.target.checked, subcontractorPriceReady: false } : { [group.ready]: e.target.checked }))} />Ej aktuellt</label>}
                <button type="button" className={`${secondary} h-11 w-11 px-0`} disabled={working || Boolean(editing)} aria-label={`Lägg till ${group.title.toLowerCase()}`} title={`Lägg till ${group.title.toLowerCase()}`} onClick={() => setEditing({ category: group.key })}><Plus size={17} /></button>
              </header>
              ...
              <div className="divide-y divide-slate-100">{lines.map((line) => <div key={line.id} className="py-3">
                <div className="flex items-start gap-2"><div className="min-w-0 flex-1 break-words"><strong className="text-sm">{line.description}</strong><p className="mt-1 text-xs text-slate-500">{line.quantity ?? '?'} {line.unit} × {line.unitCost === null ? 'Pris saknas' : money.format(line.unitCost)} · {line.markupPercent} % påslag</p><p className={`mt-1 text-xs ${line.verified ? 'text-emerald-700' : 'text-amber-800'}`}>{line.verified ? 'Kontrollerad' : 'Ej kontrollerad'} · {basisLabels[line.quantityBasis]} · {sourceLabels[line.priceSource]}</p></div>
                  <button type="button" className={`${secondary} h-11 w-11 px-0`} disabled={working || Boolean(editing)} title="Redigera kalkylrad" aria-label={`Redigera ${line.description}`} onClick={() => setEditing({ category: line.category, line })}><Pencil size={16} /></button>
                  <button type="button" className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-lg text-slate-400 hover:bg-rose-50 hover:text-rose-700 disabled:opacity-40" disabled={working || Boolean(editing)} title="Ta bort kalkylrad" aria-label={`Ta bort ${line.description}`} onClick={() => setDeleting(line.id)}><Trash2 size={16} /></button>
                </div>
              ...
              {isEditing ? <CostLineForm key={editing.line?.id ?? `new-${editing.category}`} initial={editing.line} category={editing.category} busy={working} onCancel={() => setEditing(null)} onSave={(payload) => run(() => onCostAction(editing.line ? 'update_cost_line' : 'create_cost_line', payload))} /> : null}
            </section>
          })}</div>
        </>}
      </div>
