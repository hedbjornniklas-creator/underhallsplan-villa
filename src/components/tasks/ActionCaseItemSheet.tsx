    ...
    <div className="flex h-full flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6">
        {tab === 'scope' ? <fieldset disabled={working} className="space-y-5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-semibold">Kalkylunderlag</h3><div className="flex flex-wrap gap-2">{onRequest ? <button type="button" className={secondary} disabled={working || dirty || costDirty || !item.costLines.length} onClick={() => onRequest(item.costLines.length <= 30 ? item.costLines.map((line) => line.id) : [])}><Mail size={17} />Begär offert</button> : null}<button type="button" className={primary} disabled={working || dirty || !scope.trim() || costDirty} onClick={generate}>{generating ? <Loader2 size={17} className="animate-spin" /> : <Sparkles size={17} />}{generating ? 'Skapar förslag…' : item.costSuggestion ? 'Nytt AI-förslag' : 'Föreslå kalkyl med AI'}</button></div></div>
