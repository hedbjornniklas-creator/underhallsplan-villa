    ...
    <div className="mx-auto max-w-4xl space-y-4 pb-20">
      ...
      {isDialogOpen ? (
        <div className="fixed inset-0 z-[100]">
          <button
            type="button"
            className="absolute inset-0 hidden bg-slate-950/35 md:block"
            aria-label="Stäng kontrollplats"
            onClick={() => {
              setIsDialogOpen(false)
              setDialogError(null)
            }}
          />
          <aside className="absolute inset-y-0 right-0 flex w-full flex-col border-l border-slate-200 bg-white shadow-2xl md:max-w-2xl">
            <header className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-200 px-4 py-4 md:px-5">
              ...
              <div className="space-y-3">
                ...
                {!isDialogRowPersisted ? (
                  <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                    Spara kontrollplatsen först. Öppna kontrollplatsen igen för att lägga till bilder.
                  </div>
                ) : null}
            ...
            <footer className="flex shrink-0 justify-end gap-2 border-t border-slate-200 px-4 py-3 md:px-5">
              <button
                type="button"
                onClick={() => {
                  setIsDialogOpen(false)
