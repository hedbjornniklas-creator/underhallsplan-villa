      ...
      <div className="space-y-5">
        ...
        {activeRoom && panelContent && (
          <>
            ...
            >
              <aside
                className="h-full w-full max-w-[1440px] border-l border-gray-200 bg-white shadow-2xl lg:flex lg:flex-col"
...
function RoomControlPointsSection({
  ...
  const toggleListGroup = (groupId: string) => {
    setExpandedListGroupIds(prev => {
      return prev.has(groupId) ? new Set() : new Set([groupId])
    })
  }
  ...
  function renderControlPointListGroup(group: (typeof groupedItems)[number], groupIndex: number) {
    ...
    const statusLabel = isGreen
      ? 'OK'
      : isYellow
        ? `${selectedItems.length} notering${selectedItems.length === 1 ? '' : 'ar'}`
        : 'Ej klar'
    ...
    const renderItemDetails = (
      ...
      const analysisGridClass =
        hasRiskText && hasFtuText ? 'mt-2 grid gap-2 md:grid-cols-2' : 'mt-2 grid gap-2'

      return (
        <div key={ci.id ?? label} className="space-y-2">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            {baseItem.id ? (
              <button
                ...
                className="text-xs text-gray-400 hover:text-rose-600 hover:underline md:text-[11px]"
                disabled={isInspectionLocked}
              >
                Ta bort
              </button>
              ...
              <div className="divide-y divide-gray-100 border-y border-gray-200 bg-white">
                ...
                {outcomes.map(outcome => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`h-1.5 w-1.5 rounded-full ${isActive ? 'bg-gray-900' : 'bg-transparent'}`}
                        aria-hidden="true"
                      />
                      <span>{outcome.label}</span>
                      {isActive ? <span className="text-[11px] text-gray-500">Vald</span> : <span />}
                    </button>
                  )
                })}
              ...
              {selectedItems.length === 0
                ...
                : selectedItems.map(ci => {
                    const selectedOutcome = ci.selected_outcome_id
                      ...
                      : null
                    return renderItemDetails(ci, selectedOutcome?.label ?? ci.title)
                  })}
            </div>
