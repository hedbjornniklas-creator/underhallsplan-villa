...
export default function TuReportTemplatesAdminPanel() {
  ...
  const loadAll = async () => {
    ...
    setSelectedTemplateId((current) =>
      current && nextTemplates.some((template) => template.id === current) ? current : nextTemplates[0]?.id ?? null
    )
    setLoading(false)
  }
  ...
  const saveTemplateDraft = async () => {
    ...
    if (templateDraft.id) {
      ...
      setTemplates((current) =>
        current.map((template) => (template.id === templateDraft.id ? ({ ...template, ...payload } as TemplateRow) : template))
      )
      setTemplateDraft(null)
      return
    }
    ...
    setTemplates((current) => [...current, created].sort((a, b) => a.sort_order - b.sort_order || a.title.localeCompare(b.title, 'sv')))
  ...
  const deleteTemplate = async (template: TemplateRow) => {
    ...
    if (templateDraft?.id === template.id) setTemplateDraft(null)
  }
  ...
  const openNewSection = () => {
    ...
    setSectionDraft({
      ...
      default_content: '',
      ai_instruction: '',
      sort_order: (selectedTemplateSections.length + 1) * 100,
      is_required: false,
      include_in_toc: true,
      allow_delete: true,
    })
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-left text-gray-600">
                <th className="py-2 pl-3 pr-2">Mall</th>
            ...
            <tbody className="divide-y">
              {filteredTemplates.map((template) => (
                <tr
                  key={template.id}
                  className={template.id === selectedTemplate?.id ? 'bg-violet-50/70' : undefined}
                >
                  <td className="py-2 pl-3 pr-2">
                    <button
                      ...
                      className="text-left"
                    >
                      <span className="block font-medium text-gray-950">{template.title}</span>
                      <span className="block text-xs text-gray-500">{template.key}</span>
                    </button>
                  </td>
                  <td className="py-2 pr-2">{template.is_active ? 'Ja' : 'Nej'}</td>
              ...
              {!loading && filteredTemplates.length === 0 ? (
                <tr>
                  <td className="py-4 pl-3 text-gray-500" colSpan={3}>
                    Inga mallar.
                  </td>
                </tr>
              ) : null}
                    ...
                    {selectedTemplateSections.map((section) => {
                      ...
                      return (
                        <tr key={section.id}>
                          ...
                          <td className="py-2 pr-3">{section.sort_order}</td>
