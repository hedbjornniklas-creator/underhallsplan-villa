...
import { verifyFortnoxConnection } from '@/lib/fortnox/server'
...
function invalidBody() {
  return NextResponse.json(
    {
      error: 'Begäran innehåller inte en giltig organisation.',
...
export async function POST(request: Request) {
  try {
    ...
    if (
      Object.keys(values).some((key) => key !== 'orgId') ||
    ...
    const connection = await verifyFortnoxConnection(values.orgId)
    return NextResponse.json(
      { connection },
