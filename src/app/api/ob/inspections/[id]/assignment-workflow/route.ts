...
import type { ObAssignmentWorkflow } from '@/lib/ob/assignmentWorkflow'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body?.confirmed !== true || typeof body?.reviewToken !== 'string') {
      return NextResponse.json({ error: 'Bekräfta avstämningen av kundens uppgifter och tillägg.' }, { status: 400 })
    }
    const workflow = await obWorkflowRpc<ObAssignmentWorkflow>('ob_review_assignment_workflow', {
