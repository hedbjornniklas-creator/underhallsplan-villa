...
export default async function ObLayout({ children }: { children: ReactNode }) {
  try {
    await requireModuleAccess({ productKey: 'dashboard', moduleKey: 'inspections' })
  } catch (error) {
      ...
      <main className="mx-auto w-full max-w-5xl px-6 py-12">
        <section className="rounded-lg border border-rose-200 bg-rose-50 p-8 text-rose-900">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-7">
            Överlåtelsebesiktning kräver egen modulbehörighet. Be en administratör aktivera ÖB för din användare.
          </p>
        </section>
      </main>
    )
