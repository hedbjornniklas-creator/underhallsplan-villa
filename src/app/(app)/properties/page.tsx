...
type PropertyRow = {
  ...
  status: 'Utkast' | 'Aktiv' | 'Arkiverad' | null
  created_at?: string
}
...
export default function PropertiesPage() {
  ...
  const [filter, setFilter] = useState<'Alla' | 'Utkast' | 'Aktiv' | 'Arkiverad'>('Alla')

  const load = async () => {
    setLoading(true)
    const { data, error } = await supabase
      .from('properties')
      .select('id,name,address,client_name,status,created_at')
      .order('created_at', { ascending: false })

    if (!error) setRows((data ?? []) as PropertyRow[])
    setLoading(false)
  }
  ...
  const createPropertyAndGo = async () => {
    ...
    const { data, error } = await supabase
      .from('properties')
      .insert({ name: tempName, status: 'Utkast' })
      .select('id')
      .single()
