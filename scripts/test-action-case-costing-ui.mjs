...
try {
  ...
  page.on('request', (request) => {
    ...
    const allowed = target.origin === url && request.method() === 'GET' &&
      (['/', '/view.js', '/favicon.ico'].includes(target.pathname) || /^\/api\/action-cases\/[^/]+\/attachments\/[^/]+$/.test(target.pathname) || /^\/api\/action-cases\/rfq\/[^/]+\/files\/[^/]+$/.test(target.pathname) || /^\/api\/action-cases\/[^/]+\/rfq\/[^/]+\/files\/[^/]+$/.test(target.pathname))
  ...
  for (const width of [1440, 390]) {
    start('rfq-contact-and-large-files', width)
    await page.setViewport({ width, height: 900 })
    await page.goto(`${url}/?requests=1&largeFiles=1`, { waitUntil: 'networkidle0' })
    await click('Begär offert')
    const contactId = (n) => `00000000-0000-4000-8000-${String(n).padStart(12, '0')}`
    assert.equal(await page.$$eval('[name=recipientContact] option', (nodes) => nodes.some((node) => node.textContent.includes('Inaktiv'))), false)
    await page.select('[name=recipientContact]', `profile:${contactId(81)}`)
    assert.equal(await page.$eval('[name=supplierEmail]', (node) => node.value), 'internal@example.test')
    await page.select('[name=recipientContact]', `contact:${contactId(83)}`)
    assert.equal(await page.$eval('[name=supplierEmail]', (node) => node.value), '')
    assert.equal(await page.$eval('[name=supplierEmail]', (node) => node.required), true)
    await page.select('[name=recipientContact]', `contact:${contactId(82)}`)
    assert.equal(await page.$eval('[name=supplierName]', (node) => node.value), 'Exempelbygg AB')
    assert.equal(await page.$eval('[name=supplierEmail]', (node) => node.value), 'anna@example.test')
    await page.type('[name=supplierName]', ' extra')
    assert.equal(await page.$eval('[name=recipientContact]', (node) => node.value), '')
    await page.select('[name=recipientContact]', `contact:${contactId(82)}`)
    await checkLabel('Välj alla delar')
    await page.click(`input[name=attachment][value="${contactId(31)}"]`)
    await page.click(`input[name=attachment][value="${contactId(32)}"]`)
    assert.doesNotMatch(await page.$eval('[role=dialog]', (node) => node.textContent), /5 MB/)
    await noOverflow()
    await click('Spara och förhandsgranska')
    await page.waitForSelector('[data-testid=request-body]')
    assert.equal((await json('requests'))[0].attachmentIds.length, 2)
    await click('Skicka förfrågan')
    await page.waitForFunction(() => document.body.textContent.includes('Mottagarlänk giltig till'))
    await page.screenshot({ path: join(output, `rfq-sent-${width}.png`) })
    await click('Återkalla länk')
    await click('Avbryt')
    await click('Återkalla länk')
    await click('Återkalla åtkomst')
    await page.waitForFunction(() => document.body.textContent.includes('Mottagarlänken är återkallad'))
    assert.ok((await json('requests'))[0].delivery.revokedAt)

    start('rfq-recipient-gallery', width)
    await page.goto(`${url}/?requests=1&publicRfq=1`, { waitUntil: 'networkidle0' })
    await page.waitForFunction(() => [...document.querySelectorAll('img')].some((img) => img.complete && img.naturalWidth > 0))
    assert.equal(await page.$eval('a[aria-label="Ladda ner entre.png"]', (node) => node.href.endsWith('?download=1')), true)
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true)
    await page.screenshot({ path: join(output, `rfq-gallery-${width}.png`) })
    await page.click('[aria-label="Granska entre.png"]')
    ...
    await noOverflow()
    await page.screenshot({ path: join(output, `rfq-image-${width}.png`) })
    await page.keyboard.press('ArrowRight')
    assert.match(await page.$eval('[aria-label="Granska bild"] header', (node) => node.textContent), /fonster.png/)
    await page.keyboard.press('Escape')
    assert.equal(await page.$('[aria-label="Granska bild"]'), null)
  ...
  for (const width of [1440, 390]) {
    ...
    await click('Begär offert')
    assert.equal(await page.$eval('input[name=pricePresentation][value=action_total]', (node) => node.checked), true, 'New requests default to totals per action')
    ...
    await checkLabel('Välj alla delar'); assert.deepEqual(await selectedWork(), [21, 22, 23, 24])
    assert.deepEqual(await selectedFiles(), [31, 32, 34])
    await noDuplicateAttachments()
    await page.click('input[name=attachment][value$="000000000034"]')
    await checkLabel('Välj alla delar'); assert.deepEqual(await selectedWork(), [])
    await checkLabel('Välj alla delar'); assert.deepEqual(await selectedFiles(), [31, 32], 'Select-all retains manual shared-document opt-out')
    await page.click('input[name=attachment][value$="000000000034"]')
    ...
    assert.equal(saved.pricePresentation, 'action_total')
    ...
    assert.match(groupedBody, /Arbetsdel: Panel och målning/)
    ...
    assert.equal(await page.$eval('input[name=pricePresentation][value=action_total]', (node) => node.checked), true)
    await page.click('input[name=pricePresentation][value=line_items]')
    await click('Spara och förhandsgranska')
    await page.waitForSelector('[data-testid=request-body]')
    ;[saved] = await json('requests')
    assert.equal(saved.pricePresentation, 'line_items')
    ...
    await click('Redigera')
    assert.equal(await page.$eval('input[name=pricePresentation][value=line_items]', (node) => node.checked), true)
  }
  for (const width of [1440, 390]) {
    ...
    await page.locator('summary').filter((node) => node.textContent === 'Ändra valda arbeten (2)').click()
    ...
    assert.equal(await page.$eval('input[name=costLineSelection][value$="000000000006"]', (node) => node.checked), true, 'Moving work and changing filters retain selected material')
    assert.equal(await page.$(workCheckbox('Efterkontroll')), null, 'Work-part filter hides unassigned work')
    await checkLabel('Välj alla synliga arbeten')
    await page.locator('summary').filter((node) => node.textContent === 'Ändra valda arbeten (2)').click()
    ...
    await checkLabel('Välj alla synliga arbeten')
    await selectLabel('Arbetsdel', '')
    assert.equal(await page.$eval(workCheckbox('Montering'), (node) => node.checked), true, 'Changing filters retains selected work')
    await selectLabel('Arbetsdel', partId)
    await click('Begär offert för valda (3)')
    ...
    assert.deepEqual([...requestedIds].sort(), savedItem.costLines.filter((line) => line.workPartId === partId || line.category === 'material').map((line) => line.id).sort())
    ...
    assert.ok(request.lines.every((line) => line.category === 'material' || line.workPartId === partId))
    ...
    assert.match(request.body, /Arbetsdel: Panel och ytbehandling vid entrén/)
  ...
  for (const width of [1440, 390]) {
    ...
    const original = await json('item')
    assert.equal(await page.$$eval(selection, (nodes) => nodes.length), 7, 'Every category has row checkboxes')
    assert.deepEqual(await offer(), [])
    assert.doesNotMatch(await page.$eval('[role=dialog]', (node) => node.textContent), /Timmar \/ offerter/)
    await checkLabel('Välj allt material'); assert.deepEqual(await selected(), [6, 7])
    await checkLabel('Välj alla arbeten'); assert.deepEqual(await selected(), [6, 7, 51, 52, 53])
    await checkLabel('Välj allt övrigt'); assert.deepEqual(await selected(), [6, 7, 8, 9, 51, 52, 53])
    await page.locator(`${selection}[value$="000000000007"]`).click()
    assert.equal(await page.$$eval('label', (nodes) => nodes.find((node) => node.textContent.trim() === 'Välj allt material')?.querySelector('input').indeterminate), true, 'Category checkbox shows partial selection')
    await checkLabel('Välj allt material'); assert.deepEqual(await selected(), [6, 7, 8, 9, 51, 52, 53])
    await checkLabel('Välj allt övrigt'); assert.deepEqual(await selected(), [6, 7, 51, 52, 53], 'Category deselection preserves other selections')
    await page.locator('[aria-label="Rensa valda kalkylrader"]').click()
    assert.deepEqual(await selected(), [])
    await checkLabel('Välj allt material')
    await page.locator(`${selection}[value$="000000000051"]`).click()
    await page.locator(`${selection}[value$="000000000009"]`).click()
    assert.deepEqual(await offer(), [{ text: 'Begär offert för valda (4)', disabled: false }])
    assert.deepEqual(await json('item'), original, 'Selection alone never changes costing')
    await page.locator('[aria-label="Timmar för Montering"]').fill('9')
    assert.equal((await offer())[0].disabled, true, 'Unsaved direct hours block the RFQ')
    assert.equal(await page.$$eval(selection, (nodes) => nodes.every((node) => node.disabled)), true, 'Unsaved pricing blocks selection changes')
    await page.locator('[aria-label="Återställ egen kalkyl för Montering"]').click()
    assert.equal((await offer())[0].disabled, false)
    await page.$eval(`${selection}[value$="000000000007"]`, (node) => node.scrollIntoView({ block: 'center' }))
    await noOverflow()
    assert.equal(await page.$eval('[role=dialog] footer', (node) => {
      const button = [...node.querySelectorAll('button')].find((node) => node.textContent.startsWith('Begär offert för valda'))
    ...
    }), true, 'Shared offer action stays visible and clickable while scrolling material')
    await page.screenshot({ path: resolve(output, `mixed-cost-selection-${width}.png`), fullPage: true })
    await click('Begär offert för valda (4)')
    await page.waitForSelector('#group-request-form')
    assert.deepEqual(await page.$$eval('input[name=workLine]:checked', (nodes) => nodes.map((node) => Number(node.value.slice(-3))).sort((a, b) => a - b)), [6, 7, 9, 51])
    assert.equal(await page.$eval('input[name=pricePresentation][value=action_total]', (node) => node.checked), true)
    await page.locator('input[name=supplierName]').fill('Syntetisk Bygg AB')
    await page.locator('input[name=supplierEmail]').fill('bygg@example.test')
    await click('Spara och förhandsgranska')
    await page.waitForSelector('[data-testid=request-body]')
    const [saved] = await json('requests')
    assert.deepEqual([...new Set(saved.lines.map((line) => line.category))].sort(), ['material', 'own_labor', 'transport'])
    assert.equal(saved.lines.find((line) => line.description === 'Grundfärg').quantity, null, 'Unknown material quantities can still be requested')
    assert.equal(saved.sentAt, null, 'Selection and preview do not send to a supplier')
    assert.doesNotMatch(saved.body, /175|500|550|påslag/)
    assert.deepEqual(await json('item'), original)
    ...
    await page.waitForSelector('#group-request-form')
    assert.deepEqual(await page.$$eval('input[name=workLine]:checked', (nodes) => nodes.map((node) => Number(node.value.slice(-3))).sort()), [6, 7])
    await page.locator('input[name=supplierName]').fill('Syntetisk Material AB')
    await page.locator('input[name=supplierEmail]').fill('material@example.test')
    await click('Spara och förhandsgranska')
    await page.waitForSelector('[data-testid=request-body]')
    assert.ok((await json('requests'))[0].lines.every((line) => line.category === 'material'))
    ...
    await page.locator('[aria-label="Ta bort Material 1"]').click()
    await click('Ta bort')
    await page.waitForFunction(() => !JSON.parse(document.querySelector('[data-testid=item]').textContent).costLines.some((line) => line.description === 'Material 1'))
    assert.equal((await offer())[0].text, 'Begär offert för valda (31)', 'Deleted rows no longer count towards selection')
    await page.locator(`${selection}[value$="000000000101"]`).click()
    assert.deepEqual(await offer(), [{ text: 'Begär offert för valda (30)', disabled: false }])
    await noOverflow()
  }
  for (const width of [1440, 390]) {
    start('costing', width)
  ...
  for (const width of [1440, 390]) {
    ...
    assert.match(body, /Åtgärd: Träpanel vid entrén/)
    assert.match(body, /Åtgärd: Fönsterbleck på gårdssidan/)
    ...
    assert.match(body, /Redovisa även dessa prisuppgifter separat/)
  ...
  for (const width of [1440, 390]) {
    start('mixed-action-total', width)
    await page.setViewport({ width, height: 900 })
    await page.goto(`${url}/?requests=1&parts=1&packages=1&allCosts=1`, { waitUntil: 'networkidle0' })
    await click('Begär offert')
    await checkLabel('Träpanel vid entrén')
    assert.equal(await page.$$eval('input[name=workLine]:checked', (nodes) => nodes.length), 5)
    await page.locator('input[name=supplierName]').fill('Syntetisk Totalpris AB')
    await page.locator('input[name=supplierEmail]').fill('total@example.test')
    await page.$eval('input[name=workLine]', (node) => node.scrollIntoView({ block: 'center' }))
    await noOverflow()
    await page.screenshot({ path: resolve(output, `all-cost-selection-${width}.png`), fullPage: true })
    await click('Spara och förhandsgranska')
    await page.waitForSelector('[data-testid=request-body]')
    const [request] = await json('requests')
    assert.equal(request.pricePresentation, 'action_total')
    assert.equal(new Set(request.lines.map((line) => line.itemId)).size, 1)
    assert.deepEqual([...new Set(request.lines.map((line) => line.category))].sort(), ['material', 'own_labor', 'transport'])
    for (const text of ['Material:', 'Övrigt:', 'Grundmålad panel', 'Bortforsling av avfall']) assert.ok(request.body.includes(text))
    assert.doesNotMatch(request.body, /500|1200|150|påslag/)
    await click('Skicka förfrågan')
    await page.waitForSelector('[aria-label=Grupppriser]')
    assert.equal(await page.$$eval('[aria-label=Grupppriser] > ul > li', (nodes) => nodes.length), 1, 'Multiple work parts merge into one action total')
    assert.doesNotMatch(await page.$eval('[aria-label=Offertsvar]', (node) => node.textContent), /Enskilda radpriser/)
    await click('Registrera totalpris')
    assert.equal(await page.$('input[name=independentGroupPrice]'), null, 'No artificial allocation within one action')
    assert.equal(await page.$$eval('[aria-label="Ingår i priset"] li', (nodes) => nodes.length), 5)
    await page.locator('input[name=groupAmount]').fill('10000')
    await page.locator('textarea[name=groupOfferedScope]').fill('Samtliga valda delar inklusive material och bortforsling.')
    await page.click('input[name=groupPriceChecked]')
    await noOverflow()
    await page.screenshot({ path: resolve(output, `action-total-price-${width}.png`), fullPage: true })
    await click('Använd totalpriset i kalkylen')
    await page.waitForSelector('input[name=groupAmount]', { hidden: true })
    const state = await json('request-case')
    assert.equal(state.quotePackages[0].amount, 10000)
    assert.equal(state.quotePackages[0].coveredLineIds.length, 4)
    assert.equal(state.quotePackages[0].workPartId, null)
  }
  for (const width of [1440, 390]) {
    ...
    const panelTitle = 'Träpanel vid entrén'
    const bleckTitle = 'Fönsterbleck på gårdssidan'
    await click('Begär offert')
    ...
    await click('Skicka förfrågan')
    await page.waitForSelector('[aria-label=Grupppriser]')
    await groupButton(panelTitle, 'Registrera totalpris')
    ...
    assert.equal(await page.$('input[name=groupCoveredLine]'), null, 'Total covers the originally requested material, not extra rows')
    await page.click('input[name=groupPriceChecked]')
    ...
    await groupButton(bleckTitle, 'Registrera totalpris')
