...
try {
  for (const width of [344, 390, 1440]) {
    ...
    try {
      ...
      await button('Skicka ansökan').click()
      await page.waitForFunction(() => document.body.textContent.includes('Din ansökan är skickad'))
      await page.waitForFunction(() => document.activeElement?.id === 'application-sent-heading')
