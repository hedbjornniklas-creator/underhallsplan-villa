...
const offer = { available: true, alreadyActive: false, priceOre: 59900, netPriceOre: 47920, vatOre: 11980, vatRate: 25,
...
const server = createServer(async (request, response) => {
  if (request.url === '/mock-follow-up') {
    ...
    if (request.method === 'GET') { response.end(JSON.stringify({ offer })); return }
...
try {
  ...
  for (const width of [1440, 390]) {
    ...
    await page.locator('#digital-follow-up button').filter(node => node.textContent.includes('Se vad')).click()
