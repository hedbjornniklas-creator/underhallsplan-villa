...
if (process.argv.includes('--serve')) {
  ...
  try {
    ...
    for (const width of [1440,1024,390,344]) {
      ...
      await page.click('#board-decision input[value="conditional"]')
      await (await page.$('#board-decision')).screenshot({path:resolve(output,`decision-${width}.png`)})
    ...
    assert.equal(await page.$eval('#board-decision button[type=submit]',node=>node.disabled),false)
    await page.evaluate(()=>sessionStorage.removeItem('board-last-status'))
    await page.click('#board-decision button[type=submit]')
    assert.equal(await page.evaluate(()=>sessionStorage.getItem('board-last-status')),null)
    await page.type('#board-decision textarea','   ')
    await page.click('#board-decision input[type=checkbox]')
    await page.click('#board-decision button[type=submit]')
    await page.waitForFunction(()=>document.querySelector('#board-decision').textContent.includes('Skriv en motivering till beslutet.'))
    await page.type('#board-decision textarea','Styrelsens beslut med kvarstående fråga.')
    await page.click('#board-decision button[type=submit]')
    await page.waitForFunction(()=>JSON.parse(sessionStorage.getItem('board-last-status')??'null')?.status==='approved')
    assert.match(await page.$eval(section,node=>node.textContent),/Svar inkommet/)
    assert.doesNotMatch(await page.$eval(section,node=>node.textContent),/Begäran behöver skickas/)
    console.log('PASS approval requires motivation but preserves open questions')

    await page.goto(url,{waitUntil:'networkidle0'})
    await page.click('#board-decision input[value="conditional"]')
    await page.type('#board-decision textarea','Motivering för villkorat godkännande.')
    const fields=await page.$$('#board-decision textarea')
    await fields[1].type('Arbetet får påbörjas efter styrelsens besked.')
    await page.click('#board-decision input[type=checkbox]')
    await page.click('#board-decision button[type=submit]')
    await page.waitForFunction(()=>JSON.parse(sessionStorage.getItem('board-last-status')??'null')?.status==='conditional')
    const decision=await page.evaluate(()=>JSON.parse(sessionStorage.getItem('board-last-status')))
    assert.match(decision.reason,/Motivering/)
    assert.match(decision.conditions,/Arbetet/)
    console.log('PASS conditional approval retains separate motivation and conditions')
    ...
    console.log('PASS latest checkbox choice and optional separate clarification assessment')
