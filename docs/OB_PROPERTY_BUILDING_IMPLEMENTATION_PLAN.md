...
The installed-schema rehearsal and synthetic SQL building checks now pass in
the separate Supabase test project; see `OB_STAGING_REHEARSAL.md`. Mobile/API,
Storage and report/PDF acceptance remain open. The test rollout flag is OFF.
...
- Two diagnostic-export tests verify that the revised read-only preflight emits
  all metadata in one result, works without OB tables, and exposes no row values.

### Installed-catalog review, 2026-09-12

The initial export contained only privileges. The subsequent version-2 preflight
contains all seven metadata sections: 35 tables, 426 columns, 106 indexes,
184 constraints, 58 triggers, 38 policies and 612 privilege entries on PostgreSQL
17.6. Receipt of this metadata is NOT approval to execute files `01` through `04`.
No installed grants, policies, records or files have been changed by this review.

Confirmed blockers:

- RLS is disabled on 14 tables, each granting SELECT/INSERT/UPDATE/DELETE to both
  anon and authenticated: inspection_addon_orders, inspection_area_measurements,
  inspection_area_measurement_rows, inspection_conditions,
  inspection_control_answers, inspection_control_point_answers,
  inspection_exterior_observations, inspection_exterior_selections,
  inspection_interior_observations, inspection_interior_rooms,
  inspection_lock_events, inspection_moisture_controls,
  inspection_moisture_control_rows and inspection_moisture_control_images.
- Buildings, inspection_control_items and inspection_images have enabled RLS
  but unrestricted authenticated policies (`USING (true)` / `WITH CHECK (true)`).
  Their policies do not isolate owners or organizations. Parent-table RLS does
  not automatically protect direct child-table queries; write triggers cannot
  protect SELECT. Actual API exposure, Storage policies and installed function
  bodies remain to be checked. This metadata is not evidence of exploitation.
- Both anon and authenticated have TRUNCATE privileges on 26 tables in the full
  export. RLS does not govern TRUNCATE; review execution paths and remove
  unnecessary grants as part of a separately tested access-control change.
- `inspection_overview_selections` also has the constraint
  `inspection_overview_selection_inspection_id_overview_item_i_key`, UNIQUE on
  `(inspection_id, overview_item_id, floor_key, set_index)`. At review, file `04`
  removed only the other global unique index. That left a constraint blocking the
  same overview item/floor/set in two different building parts. A local in-memory
  PGlite reproduction using the current cutover's index statements confirmed
  SQLSTATE 23505 for a second building on `plan0`. No real inspection data was
  used. The earlier representative test fixture omitted this installed rule.

Local follow-up: file `04` now validates and removes the installed constraint
after creating scoped replacements, with no CASCADE or record changes. Tests
first reproduced the failure and now cover same-floor choices in two parts,
duplicates within one part, NULL floors, legacy uniqueness and unexpected-schema
rejection. The fixture also includes the known image CASCADE/origin SET NULL
foreign keys and control-item side check from the export.

A separate access-hardening draft and role-based tests are prepared in
`2026-09-12_09_inspection_access_hardening.sql`. It is not a prerequisite script
to run blindly with `01`-`04`: it requires its own review and explicit execution
gate. The [security rollout checklist](INSPECTION_ACCESS_ROLLOUT.md) documents
the caller inventory, remaining Storage/RPC/view checks, staging, backup and
production approval. The supplied `2026-09-12_08_inspection_access_preflight.sql`
result has now been reviewed. It confirmed additional unrestricted building
media, broad Storage policies and four legacy definer RPCs callable by anon.
`09` now includes building media; the separate draft `10` restricts those RPCs
and media listing/writes. Old public image URLs remain public, intentionally not
rewritten during this security preparation. Review the remaining view/API paths
and rehearse both drafts in Supabase staging; no hardening has been applied to
production. No repeat export is needed before arranging that test environment.

Keep rollout disabled pending installed-function/access-path review and real
staging acceptance. Do not rerun dedupe scripts or change real records to bypass
uniqueness failures. Historical test counts above describe the initial build,
not the subsequent security/cutover regression coverage.

References: [PostgreSQL row security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html)
...
- Resolve the installed-catalog blockers above and inspect the installed trigger
  function bodies, API execution paths and Storage policies not in the export.
...
The implementation was committed and pushed to a separate development branch;
that is not a production deployment. No production migration, activation or
record/file changes were performed. The SQL rollout flag remains false.
...
### Staging app follow-up, 2026-09-12

An isolated real Next app now runs against `hushub-ob-test`, with staging-only
`09`/`10` permissions, two synthetic Auth identities and a synthetic property.
The UI has been used to activate the main building and add a guesthouse with its
own Plan 0 room/free note. Existing room IDs, legacy floor labels and original
note text were verified afterwards. A 390px mobile view of the actual app was
checked. Initial real-JWT/API/Storage isolation checks pass. Staging rollout is
ON for these fixtures only; production has not been modified or approved.

The test catalog now includes representative separate building conditions, free
text, a note suggestion and synthetic covers/photos. Conditions revisions and
ownership, image link/unlink safety, and a 17-page draft PDF with separated
building content have been tested. A furnishing-code presentation defect was
fixed without rewriting stored data. Photo continuation still needs repeated
building/room context. Frozen snapshots, delivery/locking, four-building workflows,
offline/device recovery and full report acceptance are still pending. See
`OB_STAGING_REHEARSAL.md` for startup, results, exclusions and remaining gates.
