...
Update 2026-09-09 (Fortnox connection)
...
- The integration uses Fortnox OAuth2 service-account consent and exactly one allowed scope, `companyinformation`. Application validation and database constraints reject both missing and additional scopes. Authorization tokens are short-lived server values and are never persisted; the verified TenantId and non-secret company metadata are stored per organization.
