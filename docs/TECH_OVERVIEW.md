...
- Deployment, sandbox, the Vercel callback-query logging release gate, Client Secret rotation, STYR pilot and troubleshooting are documented in `docs/FORTNOX_CONNECTION_OPERATIONS.md`. No real OAuth flow may pass that gate until query values are verified as redacted before log storage or a hardened callback ingress replaces the exposed platform path.
