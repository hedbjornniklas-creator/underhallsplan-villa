...
- See `docs/ADMIN_ACCESS_UI_SPEC.md` for the implementation-ready admin UI specification for `/admin/access`.

Update 2026-09-10 (Fortnox connection and scope expansion)
- `/settings` contains a multi-organization Fortnox card. Active organization members can read status; changes require both an active `org_members` admin membership and the existing exact organization-scoped Dashboard/Admin access.
- Server routes are `/api/integrations/fortnox/status`, `/organization-number`, `/connect`, `/callback` and `/verify`. Mutation start points enforce same-origin requests; the OAuth callback consumes a profile-bound, ten-minute state exactly once. Starting a newer attempt supersedes older attempts for that organization, and the database only lets the newest attempt commit a connection.
- The integration uses Fortnox OAuth2 service-account consent and requests the exact current scope set `companyinformation`, `customer` and `invoice`. The database preserves the original `companyinformation`-only grant only as `needs_reauthorization`; it cannot be treated as connected. Application validation and database constraints reject partial or additional scope sets. Authorization tokens are short-lived server values and are never persisted; the verified TenantId and non-secret company metadata are stored per organization. Customer and invoice mutations are not implemented by this connection-only release.
- A callback parses validated company information with the authorization grant, then verifies TenantId, organization number and scopes again with a fresh client-credentials token before saving the returned company name. The legal organization number must match the HusHub organization.
- `fortnox_connections` has one row per HusHub organization, a globally unique TenantId and a monotonic `connection_version`. Both base tables and all Fortnox mutation RPCs are browser-inaccessible and service-role-only; only a safe, membership-filtered API projection reaches Settings, never TenantId. OAuth state is stored only as a SHA-256 hash.
- The shown status is the last verified connection state. External consent revocation is not webhook-synchronized in this first step; administrators can run a live Client Credentials verification from Settings. Rejected organization credentials/access/permissions mark the connection `needs_reauthorization`, while transient provider failures and central application-configuration failures preserve its previous status. Verification writes use organization, TenantId and `connection_version` as a compare-and-swap guard, so a stale check cannot overwrite a reconnect or another check. The explicit reconnect flow is always available.
- Deployment, sandbox, the accepted and bounded Vercel callback-query logging risk, Client Secret rotation, STYR pilot and troubleshooting are documented in `docs/FORTNOX_CONNECTION_OPERATIONS.md`. The pilot uses the existing Vercel callback with short-lived one-time authorization codes, restricted log access, MFA, no Log Drains and immediate redirect cleanup; the risk decision must be revisited if scopes, retention, access or rollout breadth changes.
