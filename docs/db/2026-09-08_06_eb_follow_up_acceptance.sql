-- Explicit checkout approval and durable withdrawal information.
-- Apply after 2026-09-08_04_eb_follow_up_platform_seller.sql.
-- Deploy together with checkout version 2026-09-08.2; older open forms must refresh.
begin;
...
-- Consent text, hash and dates are generated by the trusted server, not the browser.
-- A BEFORE INSERT guard also rejects old clients that do not collect the new consents.
-- Existing immutable orders are deliberately not rewritten.
create or replace function public.eb_follow_up_require_acceptance() returns trigger
language plpgsql set search_path = public as $$
declare a jsonb := new.buyer_snapshot->'acceptanceSnapshot'; accepted timestamptz; deadline date; local_day date;
begin
  if new.terms_version is distinct from '2026-09-08.2'
    or new.buyer_snapshot->>'customerType' is null
    or new.buyer_snapshot->>'customerType' not in ('consumer','business')
    or jsonb_typeof(a) is distinct from 'object'
    or a->>'termsVersion' is distinct from new.terms_version
    or coalesce(length(a->>'termsText'),0) < 100
    or coalesce(a->>'termsHash','') !~ '^[a-f0-9]{64}$'
    or coalesce(a->>'acceptedAt','') = ''
    or coalesce(a->>'withdrawalFormUrl','') = ''
    or a->'consents'->'acceptTerms' is distinct from 'true'::jsonb
    or a->'consents'->'requestImmediateStart' is distinct from 'true'::jsonb
    or a->'consents'->'acceptInvoice' is distinct from 'true'::jsonb
    or coalesce(length(a->'consentTexts'->>'acceptTerms'),0) < 10
    or coalesce(length(a->'consentTexts'->>'requestImmediateStart'),0) < 10
    or coalesce(length(a->'consentTexts'->>'acceptInvoice'),0) < 10 then
    raise exception 'EB_FOLLOW_UP_CONSENT_REQUIRED';
  end if;
  accepted := (a->>'acceptedAt')::timestamptz;
  if not isfinite(accepted) or accepted < clock_timestamp() - interval '5 minutes'
    or accepted > clock_timestamp() + interval '1 minute' then
    raise exception 'EB_FOLLOW_UP_ACCEPTANCE_TIME_INVALID';
  end if;
  local_day := (accepted at time zone 'Europe/Stockholm')::date;
  if new.buyer_snapshot->>'customerType' = 'consumer' then
    if a->'consents'->'consumerWithdrawalAcknowledged' is distinct from 'true'::jsonb
      or coalesce(length(a->'consentTexts'->>'consumerWithdrawalAcknowledged'),0) < 10
      or coalesce(a->>'withdrawalDeadline','') !~ '^\d{4}-\d{2}-\d{2}$' then
      raise exception 'EB_FOLLOW_UP_CONSENT_REQUIRED';
    end if;
    deadline := (a->>'withdrawalDeadline')::date;
    -- The server extends weekends/holidays; this range prevents a shortened or arbitrary period.
    if deadline < local_day + 14 or deadline > local_day + 21 then
      raise exception 'EB_FOLLOW_UP_ACCEPTANCE_TIME_INVALID';
    end if;
  elsif a->>'withdrawalDeadline' is not null
    or a->'consents'->'consumerWithdrawalAcknowledged' is distinct from 'false'::jsonb then
    raise exception 'EB_FOLLOW_UP_CONSENT_REQUIRED';
  end if;
  new.accepted_at := accepted;
  return new;
end $$;
drop trigger if exists eb_follow_up_acceptance_required on public.eb_follow_up_orders;
create trigger eb_follow_up_acceptance_required before insert on public.eb_follow_up_orders
  for each row execute function public.eb_follow_up_require_acceptance();
revoke all on function public.eb_follow_up_require_acceptance() from public,anon,authenticated;
