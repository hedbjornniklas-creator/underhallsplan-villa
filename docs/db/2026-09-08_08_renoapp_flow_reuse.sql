-- Reuse a complete branch by linking its existing object; never clone definitions.
-- Requires 2026-09-08_03_renoapp_flow_editor.sql. No case or configuration data is migrated.
begin;
set local lock_timeout = '5s';

create or replace function public.renoapp_edit_flow_connection(
  p_operation text,
...
declare
  ...
  result_value jsonb;
begin
  if p_operation is null or p_operation not in ('move', 'copy', 'remove') or p_apply is null
    or (p_operation <> 'remove' and (p_target_kind is null or p_target_kind not in ('action', 'option', 'document', 'participant')))
    or (p_operation = 'remove' and (p_target_kind is not null or p_target_id is not null)) then
  ...
  if p_operation <> 'remove' then
    ...
    -- Refuse rather than silently losing those settings during a move or reuse.
  ...
  version_value := md5(jsonb_build_array(p_operation, p_source_kind, p_source_id, p_source_parent_id,
    p_target_kind, p_target_id, snapshot)::text);
  ...
  result_value := jsonb_build_object('version', version_value, 'itemLabel', item_label,
    ...
    'shared', source_row.parent_kind <> 'action' or coalesce(p_target_kind <> 'action', false));
...
revoke all on function public.renoapp_edit_flow_connection(text,text,uuid,uuid,text,uuid,text,boolean) from public, anon, authenticated;
grant execute on function public.renoapp_edit_flow_connection(text,text,uuid,uuid,text,uuid,text,boolean) to service_role;

-- Preserve the existing move endpoint while sharing validation and atomic writes.
...
as $wrapper$
  select public.renoapp_edit_flow_connection('move', p_source_kind, p_source_id,
    p_source_parent_id, p_target_kind, p_target_id, p_expected_version, p_apply);
$wrapper$;
