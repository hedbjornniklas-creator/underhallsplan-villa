...
-- The installed database also has a constraint-owned global index. Replace it
-- only after both scoped indexes exist; never cascade or change existing rows.
do $$
declare installed record;
begin
  select c.contype, array(select a.attname::text from unnest(c.conkey) with ordinality k(attnum, position)
    join pg_attribute a on a.attrelid=c.conrelid and a.attnum=k.attnum order by k.position) as columns
    into installed from pg_constraint c
    where c.conrelid='public.inspection_overview_selections'::regclass
      and c.conname='inspection_overview_selection_inspection_id_overview_item_i_key';
  if found then
    if installed.contype <> 'u' or installed.columns is distinct from
      array['inspection_id','overview_item_id','floor_key','set_index']::text[] then
      raise exception 'OB_BUILDING_UNEXPECTED_OVERVIEW_CONSTRAINT';
    end if;
    alter table public.inspection_overview_selections
      drop constraint inspection_overview_selection_inspection_id_overview_item_i_key;
  end if;
end $$;
